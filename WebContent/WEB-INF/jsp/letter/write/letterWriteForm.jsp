            ...
            <div class="col-md-12">
                <div class="portlet box blue-hoki ">
                    <div class="portlet-title">
                        <div class="caption">우체통 글쓰기(입력)</div>
                    </div>
                    <div class="portlet-body">
                    	<form name="fileFrm" id="fileFrm" class="form-horizontal" method="post" enctype="multipart/form-data" action="<c:url value='/mailbox/letterSave.do'/>">
				        <div class="row">
                           	<div class="col-md-12">
                           		<div class="form-group">
                           			<label class="col-md-1 control-label">보내는사람</label>
                                   	<div class="col-md-9">
                                    	<input id="sender_id" name="sender_id" type="hidden" class="form-control input-sm" placeholder="" value="" readOnly>
                                        <input id="sender_name" name="sender_name" type="text" class="form-control input-sm" placeholder="" value="" readOnly>
                                    </div>
                                    <div class="col-md-2">
                               	...
                               	<div class="form-group">                         
                                   	...
                                   	<div class="col-md-11">
                                       	<input id="letter_receiver_id" name="letter_receiver_id" type="hidden" class="form-control input-sm" placeholder="" value="" readOnly>
                                       	<input id="letter_receiver_name" name="letter_receiver_name" type="text" class="form-control input-sm" placeholder="" value="" readOnly>
                                   	</div>                                                                
                               	</div>
								<div class="form-group">
                           			<label class="col-md-1 control-label">제목</label>
                                   	<div class="col-md-11">
                                       	<input id="letter_title" name="letter_title" type="text" class="form-control input-sm" placeholder="" value="">
                                   	</div>
                           		</div>
                                    ...
                                    <div class="col-md-11">
                                    	...
                                    	<c:forEach var="letterSkin" items="${letterSkinList }" varStatus="status">
                                    		<label class="mt-checkbox">
                                    			...
                                    			<input id="letter_skin_id" name="letter_skin_id" type="radio" class="input-sm" value="${letterSkin.letter_skin_id }">
                                    			<span></span>
                                    		</label>
                                    	</c:forEach>
                                    </div>
...
function fnLetterSenderSearchReset() {
	$('#sender_id').val('');
	$('#sender_name').val('');
	$('#letter_receiver_id').val('');
	$('#letter_receiver_name').val('');
}
...
function fnSubmit() {
	...
	if($('#sender_id').val() === '' || $('#sender_name').val() === '') {
		alert('보내는 사람을 입력하여 주시기 바랍니다.');
		return;
	}
	
	if($('#letter_title').val().trim() === '') {
		alert('제목을 입력하여 주시기 바랍니다.');
		return;
	}
	
	if(CKEDITOR.instances.letter_content.getData().trim() === '') {
	...
	var letter_status = $(':radio[name="letter_status"]:checked').val();
	if(letter_status === 'return' || letter_status === 'sendStay') {
		if(CKEDITOR.instances.letter_return_reason.getData().trim() === '') {
			alert('반송사유를 입력하여 주시기 바랍니다.');
			return;
		}	
	}
