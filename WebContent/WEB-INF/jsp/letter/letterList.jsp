        ...
        <div class="row" style="margin-top: 70px;">
        	<div class="col-md-12">
				...
				<div class="portlet-body">
                	<form name="schFrm" id="schFrm" method="post" action="<c:url value="/mailbox/letterList.do"/>">
                		<input type="hidden" name="page" id="page" value="${param.page}"/>
                      	<table class="table table-bordered">
                      		...
                      		<tr>
                      			<td style="text-align: left;">
                      				상태 
									<select class="post_select" id="status" name="status">
										<option value="">전체 </option>
										<option value="notApproval" <c:if test="${param.status eq 'notApproval'}"> selected </c:if> >미승인</option>
										<option value="approval" <c:if test="${param.status eq 'approval'}"> selected </c:if> >승인</option>
										<option value="return" <c:if test="${param.status eq 'return'}"> selected </c:if> >반송</option>
										<option value="sendStay" <c:if test="${param.status eq 'sendStay'}"> selected </c:if> >발송대기</option>
									</select>
                      			</td>
                      		</tr>
                      	</table>     
...
function fnLetterWriteForm(){
	location.href="<c:url value='/mailbox/letterWriteForm.do'/>";
}

function fnLetterViewForm(letter_id) {
	$("#letter_id").val(letter_id);
	$('#Frm').attr("action","/mailbox/letterView.do");
	$('#Frm').submit();
}

function fnLetterUpdateForm(letter_id) {
	$("#letter_id").val(letter_id);
	$('#Frm').attr("action","/mailbox/letterUpdateForm.do");
	$('#Frm').submit();
}
