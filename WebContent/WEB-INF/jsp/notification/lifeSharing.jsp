              ...
              <div class="col-md-12">
                  ...
                  <div class="portlet light portlet-fit bordered">
                      <div class="portlet-title">
                          <div class="caption">
                              <span class="caption-subject font-dark bold uppercase">생명나눔 스토리</span>
                              </div>
                              <div class="actions">
                              <div class="btn-group">
                                  <a class="btn dark btn-outline btn-circle btn-sm" href="/notification/lifeSharingWriteForm.do">생명나눔 스토리 등록</a>                                  
                              </div>
                          </div>    
                      </div>
                      <div class="portlet-body">
                      	<form name="schFrm" id="schFrm" method="post" action="<c:url value="/notification/lifeSharing.do"/>">
                                  ...
                                  <tbody>
									<c:forEach var="ntcList" items="${ntcList_list}" varStatus="status">                                      
	                                      <tr>
                                          	...
                                          	<c:forEach var="fileList" items="${fileList_list}" varStatus="status">
                                          		<c:if test="${ntcList.brd_cont_no eq fileList.brd_cont_no}">
                                          			<td style="line-height:60px">${fileList.brd_file_org_nm}</td>
                                          		</c:if>                                         		
											</c:forEach>                                          	                                                                                                                            
                                          	...
                                          	<a style="line-height:60px" onclick="lifeSharingUpdateForm(${ntcList.brd_no},${ntcList.brd_cont_no});"><button class="btn btn-default btn-circle mt-sweetalert" type="button"><i class="fa fa-paste"></i> 수정</button></a>
                                          	<a style="line-height:60px" onclick="lifeSharingDeleteEnd(${ntcList.brd_no},${ntcList.brd_cont_no});"><button class="btn btn-danger btn-circle mt-sweetalert" type="button"><i class="fa fa-times"></i> 삭제</button></a>
                                          	</td>
...
function lifeSharingUpdateForm(no,cont){
	$("#brd_no").val(no);
	$("#brd_cont_no").val(cont);
	$('#Frm').attr("action","/notification/lifeSharingUpdateForm.do");
	$('#Frm').submit();
}
function lifeSharingDeleteEnd(no,cont) {
    if(confirm('정말 삭제하시겠습니까? ')){
    	$.ajax({
            url : "/notification/lifeSharingDeleteEnd.do",
            type: "POST",
            data:{
                brd_no : no,
                brd_cont_no : cont
