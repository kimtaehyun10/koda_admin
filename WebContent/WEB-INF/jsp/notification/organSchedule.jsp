              ...
              <div class="col-md-12">
                  ...
                  <div class="portlet light portlet-fit bordered">
                      <div class="portlet-title">
                          <div class="caption">
                              <span class="caption-subject font-dark bold uppercase">기관일정</span>
                              </div>
                              <div class="actions">
                              <div class="btn-group">
                                  <a class="btn dark btn-outline btn-circle btn-sm" href="/notification/organScheduleWriteForm.do">기관일정 등록</a>                                  
                              </div>
                          </div>    
                      </div>
                      <div class="portlet-body">
                      	<form name="schFrm" id="schFrm" method="post" action="<c:url value="/notification/organSchedule.do"/>">
                      	...
                      	<table class="table table-bordered">
                      		...
                      		<tr>
                      			...
                      			<td style="text-align:left;" colspan="3">
									<input type="text" size="20" name="search_start_date" id="search_start_date" class="form_datetime" value="${param.search_start_date}" placeholder="시작일">	                                                           
	                                ~	                                        
									<input type="text" size="20" name="search_end_date"  id="search_end_date" class="form_datetime" value="${param.search_end_date}" placeholder="종료일">
									<div class="btn-group">
                      ...
                      <div class="portlet-body">
                          <div class="table-scrollable">
                              <table class="table table-hover">
                                  ...
                                  <tbody>
									<c:forEach var="osdList" items="${osdList_list}" varStatus="status">                                      
                                      <tr>
                                      	  <td>${(total_count - status.index) - ( (currentPage - 1)  *  displayNum ) }</td>                    	                                                                                 
                                          <td style="line-height:60px; text-align: left;">${osdList.sch_title}</td>
                                          <td style="line-height:60px">${osdList.sch_start_dt}</td>
                                          <td style="line-height:60px">${osdList.sch_end_dt}</td>
                                          <td style="line-height:60px">${osdList.sch_link_title}</td>                                                                                                                       
                                          <td style="line-height:60px">${fn:substring(osdList.sch_reg_dm,0,10)}</td>                                                                               
                                          <td style="line-height:60px">${osdList.sch_read_num}</td>                                                                               
                                          <td style="line-height:60px">
                                          <a style="line-height:60px" onclick="organScheduleUpdateForm(${osdList.sch_no});"><button class="btn btn-default btn-circle mt-sweetalert" type="button"><i class="fa fa-paste"></i> 수정</button></a>
                                          <a style="line-height:60px" onclick="organScheduleDeleteEnd(${osdList.sch_no})"><button class="btn btn-danger btn-circle mt-sweetalert" type="button"><i class="fa fa-times"></i> 삭제</button></a>
                                          </td>
...
function organScheduleUpdateForm(no){
	$("#sch_no").val(no);
	$('#Frm').attr("action","/notification/organScheduleUpdateForm.do");
	$('#Frm').submit();
}
function organScheduleDeleteEnd(no) {
    if(confirm('정말 삭제하시겠습니까? ')){
    	$.ajax({
            url : "/notification/organScheduleDeleteEnd.do",
            type: "POST",
            data:{
                sch_no : no                
            },
