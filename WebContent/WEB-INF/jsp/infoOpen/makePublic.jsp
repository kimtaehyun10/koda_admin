              ...
              <div class="col-md-12">
                  ...
                  <div class="portlet light portlet-fit bordered">
                      <div class="portlet-title">
                          <div class="caption">
                              <span class="caption-subject font-dark bold uppercase">사전정보 공개</span>
                              </div>
                              <div class="actions">
                              <div class="btn-group">
                                  <a class="btn dark btn-outline btn-circle btn-sm" href="/infoOpen/makePublicWriteForm.do">사전정보 공개 등록</a>                                  
                              </div>
                          </div>    
                      </div>
                      <div class="portlet-body">
                      	<form name="schFrm" id="schFrm" method="post" action="<c:url value="/infoOpen/makePublic.do"/>">
                                  ...
                                  <tbody>
									<c:forEach var="ntcList" items="${ntcList_list}" varStatus="status">                                      
                                      <tr>
                                          ...
                                          <td style="line-height:60px">${ntcList.brd_etc1}</td>                                                                                                                             
                                          <td style="line-height:60px">${ntcList.brd_etc2}</td>                                                                                                                                                                                                                                                                                                    
                                          <td style="line-height:60px">${fn:substring(ntcList.reg_dm,0,10)}</td>                                                                               
                                          <td style="line-height:60px">${ntcList.brd_read_num}</td>                                                                               
                                          <td style="line-height:60px">
                                          <a style="line-height:60px" onclick="makePublicUpdateForm(${ntcList.brd_no},${ntcList.brd_cont_no});"><button class="btn btn-default btn-circle mt-sweetalert" type="button"><i class="fa fa-paste"></i> 수정</button></a>
                                          <a style="line-height:60px" onclick="makePublicDeleteEnd(${ntcList.brd_no},${ntcList.brd_cont_no});"><button class="btn btn-danger btn-circle mt-sweetalert" type="button"><i class="fa fa-times"></i> 삭제</button></a>
                                          </td>
...
function makePublicUpdateForm(no,cont){
	$("#brd_no").val(no);
	$("#brd_cont_no").val(cont);
	$('#Frm').attr("action","/infoOpen/makePublicUpdateForm.do");
	$('#Frm').submit();
}
function makePublicDeleteEnd(no,cont) {
    if(confirm('정말 삭제하시겠습니까? ')){
    	$.ajax({
            url : "/infoOpen/makePublicDeleteEnd.do",
            type: "POST",
            data:{
            	brd_no : no,
                brd_cont_no : cont
