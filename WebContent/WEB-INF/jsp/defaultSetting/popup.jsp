...
/*
	-팝업관리
	1.각 팝업에 대한 위치와 크기를 관리자가 직접 작성해서 저장하는 방식으로 만들었으나 
	한국장기조직기증원 요청사항으로 관리자가 위치 크기를 조정하지 않고 고정된 값으로 홈페이지 
	화면에 띄어달라는 하여 현재 주석처리함 추후 수정 필요
*/
%>
<!DOCTYPE html>
              ...
              <div class="col-md-12">
                  ...
                  <div class="portlet light portlet-fit bordered">
                      <div class="portlet-title">
                          <div class="caption">
                              <span class="caption-subject font-dark bold uppercase">팝업관리</span>
                              </div>
                              <div class="actions">
                              <div class="btn-group">
                                  <a class="btn dark btn-outline btn-circle btn-sm" href="/defaultSetting/popupWriteForm.do">팝업관리 등록</a>                                  
                              </div>
                          </div>    
                      </div>
                      <div class="portlet-body">
                      	<form name="schFrm" id="schFrm" method="post" action="<c:url value="/defaultSetting/popup.do"/>">
                      		...
                      		<tr>
                      			...
                      			<td style="text-align:left;">
                      				<select name="search_gubun" id="search_gubun" style="width:200px;">
                      					...
                      					<option value="Y" ${param.search_gubun=='Y'?'selected':''}>사용</option>
                      					<option value="N" ${param.search_gubun=='N'?'selected':''}>미사용</option>                      					                      					                     					
                      				</select>
                      			</td>
                      ...
                      <input type="hidden" name="ecm_seq" id="ecm_seq" value="">                                            
                      ...
                      <div class="portlet-body">
                          <div class="table-scrollable">
                              <table class="table table-hover">
                                  ...
                                  <tbody>
									<c:forEach var="popupList" items="${popupList_list}" varStatus="status">                                      
                                      <tr>
                                      	  <td>${(total_count - status.index) - ( (currentPage - 1)  *  displayNum ) }</td>                    	                                                                                 
                                          <td style="line-height:60px">${popupList.ecm_nm}</td>                                                                                   
                                          <%-- <td style="line-height:60px">
                                          	가로 : ${popupList.ecm_top} px <br>
                                          	세로 : ${popupList.ecm_left} px
                                          </td>
                                          <td style="line-height:60px">
                                          	가로 : ${popupList.ecm_width} px <br>
                                          	세로 : ${popupList.ecm_height} px
                                          </td> --%>                                        
                                          <td style="line-height:60px">${popupList.use_yn == 'Y'?'사용':'미사용'}</td>
                                          <td style="line-height:60px">${fn:substring(popupList.reg_dm,0,10)}</td>                                                                               
                                          <td style="line-height:60px">
                                          <a style="line-height:60px" onclick="popupUpdateForm(${popupList.ecm_seq});"><button class="btn btn-default btn-circle mt-sweetalert" type="button"><i class="fa fa-paste"></i> 수정</button></a>
                                          <a style="line-height:60px" onclick="popupDeleteEnd(${popupList.ecm_seq});"><button class="btn btn-danger btn-circle mt-sweetalert" type="button"><i class="fa fa-times"></i> 삭제</button></a>
                                          </td>
...
function popupUpdateForm(seq){
	$("#ecm_seq").val(seq);
	$('#Frm').attr("action","/defaultSetting/popupUpdateForm.do");
	$('#Frm').submit();
}
function popupDeleteEnd(seq) {
    if(confirm('정말 삭제하시겠습니까? ')){
    	$.ajax({
            url : "/defaultSetting/popupDeleteEnd.do",
            type: "POST",
            data:{
            	ecm_seq : seq
            },
